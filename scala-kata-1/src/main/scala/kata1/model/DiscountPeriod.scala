...
case class DiscountPeriod(startHour: Int, endHour: Int) {
  ...
  def hasIntersection(drive: HighwayDrive): Boolean = {
    ...
    val offset: Long = if (drive.enteredAt.toLocalTime.isAfter(endTime)) 1 else 0
    val discountStart = drive.enteredAt.toLocalDate.plusDays(offset).atTime(startTime)
    val discountEnd = drive.exitedAt.toLocalDate.plusDays(offset).atTime(endTime)
    ...
    !(drive.enteredAt.isAfter(discountEnd) || drive.exitedAt.isBefore(discountStart))
  }

  /** 時間帯中に料金所を通過したか */
  def isPassedThroughBy(drive: HighwayDrive): Boolean = {
    val enterTime = drive.enteredAt.toLocalTime
    val enterCondition = !(startTime.isAfter(enterTime) || endTime.isBefore(enterTime))
    val exitTime = drive.exitedAt.toLocalTime
    val exitCondition = !(startTime.isAfter(exitTime) || endTime.isBefore(exitTime))
    enterCondition || exitCondition
  }

  def isHoliday(drive: HighwayDrive): Boolean = {
    val offset: Long = if (drive.enteredAt.toLocalTime.isBefore(endTime)) 0 else 1
